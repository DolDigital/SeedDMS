		...
		if($clipboard['folders']) {
			foreach($clipboard['folders'] as $folderid) {
				...
				if($folder = $dms->getFolder($folderid)) {
					...
					$content .= "<td><a draggable=\"false\" href=\"../out/out.ViewFolder.php?folderid=".$folder->getID()."&showtree=".showtree()."\"><img draggable=\"false\" src=\"".$this->imgpath."folder.svg\" width=\"24\" height=\"24\" border=0></a></td>\n";
					$content .= "<td><a draggable=\"false\" href=\"../out/out.ViewFolder.php?folderid=".$folder->getID()."&showtree=".showtree()."\">" . htmlspecialchars($folder->getName()) . "</a>";
					if($comment) {
			...
			foreach($clipboard['docs'] as $docid) {
				...
				if($document = $dms->getDocument($docid)) {
					...
					if($latestContent = $document->getLatestContent()) {
						...
						$content .= "<td><a draggable=\"false\" href=\"../out/out.ViewDocument.php?documentid=".$docid."&showtree=".showtree()."\">" . htmlspecialchars($document->getName()) . "</a>";
						if($comment) {
							$content .= "<br /><span style=\"font-size: 85%;\">".htmlspecialchars($comment)."</span>";
						}
						$content .= "</td>\n";
